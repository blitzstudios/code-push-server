...
export class PackageDiffer {
  ...
  public generateDiffPackageMap(
    ...
    if (!newPackage || !newPackage.blobUrl || !newPackage.manifestBlobUrl) {
      console.info(`[generateDiffPackageMap] Missing required package info. blobUrl: ${!!newPackage?.blobUrl}, manifestBlobUrl: ${!!newPackage?.manifestBlobUrl}`);
    ...
    console.info(`[generateDiffPackageMap] Starting for package ${newPackage.label}, hash: ${newPackage.packageHash}`);
    console.info(`[generateDiffPackageMap] Manifest URL: ${newPackage.manifestBlobUrl}`);
    ...
    console.info(`[generateDiffPackageMap] Promises created for manifest, history, and download`);
    ...
    return q
      ...
      .spread((newManifest: PackageManifest, history: storageTypes.Package[], downloadedArchiveFile: string) => {
        ...
        console.info(`[generateDiffPackageMap] Got manifest: ${newManifest ? 'yes' : 'no'}, history: ${history?.length || 0} packages, downloaded file: ${!!downloadedArchiveFile}`);
        
        if (!newManifest) {
          console.info(`[generateDiffPackageMap] No manifest available, cannot generate diffs`);
          return [];
        }
        
        const fileMap = newManifest.toMap();
        const fileCount = Object.keys(fileMap || {}).length;
        console.info(`[generateDiffPackageMap] Manifest contains ${fileCount} files`);
        ...
        console.info(`[generateDiffPackageMap] Found ${packagesToDiff?.length || 0} packages to diff against`);
        
        const diffBlobInfoPromises: Promise<DiffBlobInfo>[] = [];
        if (packagesToDiff) {
          packagesToDiff.forEach((appPackage: storageTypes.Package) => {
            console.info(`[generateDiffPackageMap] Adding diff task for package ${appPackage.label}, hash: ${appPackage.packageHash}`);
      ...
      .then((diffBlobInfoList: DiffBlobInfo[]) => {
        ...
        console.info(`[generateDiffPackageMap] Processed ${diffBlobInfoList?.length || 0} diffs`);
        ...
        if (diffBlobInfoList && diffBlobInfoList.length) {
          ...
          diffBlobInfoList.forEach((diffBlobInfo: DiffBlobInfo) => {
            if (diffBlobInfo && diffBlobInfo.blobInfo) {
              ...
              console.info(`[generateDiffPackageMap] Added diff for package hash ${diffBlobInfo.packageHash}, size: ${diffBlobInfo.blobInfo.size} bytes`);
            }
          });

          console.info(`[generateDiffPackageMap] Returning diff package map with ${Object.keys(diffPackageMap || {}).length} entries`);
          return diffPackageMap;
        } else {
          console.info(`[generateDiffPackageMap] No diff packages were generated`);
  ...
  public generateDiffArchive(oldManifest: PackageManifest, newManifest: PackageManifest, newArchiveFilePath: string): Promise<string> {
    return Promise<string>(
      (resolve: (value?: string | Promise<string>) => void, reject: (reason: any) => void, notify: (progress: any) => void): void => {
        console.info(`[generateDiffArchive] Starting generation`);
        
        if (!oldManifest || !newManifest) {
          console.info(`[generateDiffArchive] Missing manifests - oldManifest: ${!!oldManifest}, newManifest: ${!!newManifest}`);
          resolve(null);
          return;
        }

        const diff: IArchiveDiff = PackageDiffer.generateDiff(oldManifest.toMap(), newManifest.toMap());
        console.info(`[generateDiffArchive] Generated diff - deletedFiles: ${diff.deletedFiles.length}, newOrUpdatedFiles: ${diff.newOrUpdatedEntries.size}`);

        if (diff.deletedFiles.length === 0 && diff.newOrUpdatedEntries.size === 0) {
          console.info(`[generateDiffArchive] No differences found, skipping archive creation`);
          resolve(null);
          return;
        }

        PackageDiffer.ensureWorkDirectoryExists();
        console.info(`[generateDiffArchive] Work directory exists`);

        const diffFilePath = path.join(PackageDiffer.WORK_DIRECTORY_PATH, "diff_" + PackageDiffer.randomString(20) + ".zip");
        console.info(`[generateDiffArchive] Will create diff archive at: ${diffFilePath}`);
        ...
        diffFile.outputStream.pipe(writeStream).on("close", (): void => {
          console.info(`[generateDiffArchive] Successfully created diff archive: ${diffFilePath}`);
          resolve(diffFilePath);
        });

        const json: string = JSON.stringify({ deletedFiles: diff.deletedFiles });
        console.info(`[generateDiffArchive] Added manifest with ${diff.deletedFiles.length} deleted files`);
        ...
        // with large files
        console.info(`[generateDiffArchive] Opening source archive: ${newArchiveFilePath}`);
        yauzl.open(newArchiveFilePath, { lazyEntries: true }, (err?: Error, zipFile?: any): void => {
          ...
          zipFile
              ...
              for (const [fileNameHash, fileHash] of diff.newOrUpdatedEntries) {
                if (fileNameHash === fileName) {
                  zipFile.openReadStream(entry, (entryErr?: Error, readStream?: stream.Readable): void => {
                    ...
                    console.info(`[generateDiffArchive] Adding updated/new file to diff: ${fileName}`);
            ...
            .on("end", (): void => {
              ...
              console.info(`[generateDiffArchive] Finalizing diff archive`);
              diffFile.end();
            })
  ...
  private uploadAndGetDiffBlobInfo(
    ...
    console.info(`[uploadAndGetDiffBlobInfo] Starting for package ${appPackage.label}, hash: ${appPackage.packageHash}`);
    ...
    if (!appPackage || appPackage.packageHash === newPackageHash) {
      ...
      console.info(`[uploadAndGetDiffBlobInfo] Skipping - ${!appPackage ? 'no package' : 'same hash'}`);
      return q<DiffBlobInfo>(null);
    }

    console.info(`[uploadAndGetDiffBlobInfo] Getting manifest for package ${appPackage.label}`);
    return this.getManifest(appPackage)
      .then((existingManifest?: PackageManifest) => {
        console.info(`[uploadAndGetDiffBlobInfo] Got manifest for old package: ${existingManifest ? 'yes' : 'no'}`);
        if (existingManifest) {
          const fileCount = Object.keys(existingManifest.toMap() || {}).length;
          console.info(`[uploadAndGetDiffBlobInfo] Old manifest contains ${fileCount} files`);
        }
        
        console.info(`[uploadAndGetDiffBlobInfo] Generating diff archive`);
      ...
      .then((diffArchiveFilePath?: string): Promise<storageTypes.BlobInfo> => {
        console.info(`[uploadAndGetDiffBlobInfo] Diff archive generated: ${diffArchiveFilePath ? 'yes' : 'no'}`);
        
        if (diffArchiveFilePath) {
          console.info(`[uploadAndGetDiffBlobInfo] Uploading diff archive blob`);
          return this.uploadDiffArchiveBlob(security.generateSecureKey(accountId), diffArchiveFilePath);
        }

        console.info(`[uploadAndGetDiffBlobInfo] No diff archive to upload`);
        return q(<storageTypes.BlobInfo>null);
      })
      .then((blobInfo: storageTypes.BlobInfo) => {
        if (blobInfo) {
          console.info(`[uploadAndGetDiffBlobInfo] Uploaded blob, size: ${blobInfo.size} bytes`);
          return { packageHash: appPackage.packageHash, blobInfo: blobInfo };
        } else {
          console.info(`[uploadAndGetDiffBlobInfo] No blob info available`);
          return q<DiffBlobInfo>(null);
        }
      })
      .catch((error) => {
  ...
  private getManifest(appPackage: storageTypes.Package): Promise<PackageManifest> {
    return Promise<PackageManifest>(
      (resolve: (manifest: PackageManifest) => void, reject: (error: any) => void, notify: (progress: any) => void): void => {
        console.info(`[getManifest] Starting for package ${appPackage?.label || 'unknown'}`);
        
        if (!appPackage || !appPackage.manifestBlobUrl) {
          console.info(`[getManifest] No package or manifest URL, returning null`);
          resolve(null);
          return;
        }

        console.info(`[getManifest] Downloading manifest from URL: ${appPackage.manifestBlobUrl}`);
        ...
        req.end((err, res) => {
          ...
          if (!res.text) {
            ...
            console.info(`[getManifest] Response status: ${res.status}, type: ${res.type}, headers:`, res.headers);
            resolve(null);
            return;
          }

          try {
            console.info(`[getManifest] Downloaded manifest (${res.text.length} bytes), content-type: ${res.type}`);
            console.info(`[getManifest] Manifest first 200 chars: ${res.text.substring(0, 200)}...`);
            
            const manifest = PackageManifest.deserialize(res.text);
            if (manifest) {
              const fileMap = manifest.toMap();
              const fileCount = Object.keys(fileMap || {}).length;
              console.info(`[getManifest] Successfully parsed manifest with ${fileCount} files`);
              if (fileCount > 0) {
                const sampleFiles = Object.keys(fileMap).slice(0, 3);
                console.info(`[getManifest] Sample files: ${sampleFiles.join(', ')}${fileCount > 3 ? '...' : ''}`);
              } else {
            ...
            console.info(`[getManifest] Raw content that failed parsing: ${res.text.substring(0, 500)}...`);
            resolve(null);
          }
        });
      }
  ...
  private getPackagesToDiff(
    ...
    console.info(`[getPackagesToDiff] Starting with ${history?.length || 0} packages in history`);
    console.info(`[getPackagesToDiff] App version: ${appVersion}, new hash: ${newPackageHash}, new label: ${newPackageLabel}`);
    
    if (!history || !history.length) {
      console.info(`[getPackagesToDiff] No history available`);
      return null;
    }
    ...
    const matchingAppVersionPackages: storageTypes.Package[] = history.filter((item: storageTypes.Package) => {
      ...
      if (!matchingVersion) {
        console.info(`[getPackagesToDiff] Skipping package ${item.label} - app version mismatch: ${item.appVersion} vs ${appVersion}`);
      }
      if (alreadyHasDiff) {
        console.info(`[getPackagesToDiff] Skipping package ${item.label} - already has diff for hash ${newPackageHash}`);
      }
      
      return matchingVersion && !alreadyHasDiff;
    });

    console.info(`[getPackagesToDiff] Found ${matchingAppVersionPackages.length} packages with matching app version`);
    ...
    if (matchingAppVersionPackages.length) {
      ...
      console.info(`[getPackagesToDiff] Will diff against ${packagesToProcess.length} packages (max limit: ${this._maxPackagesToDiff})`);
      packagesToProcess.forEach(pkg => {
        console.info(`[getPackagesToDiff] - Package to diff: ${pkg.label}, hash: ${pkg.packageHash}`);
      });
      
      return packagesToProcess;
    }

    console.info(`[getPackagesToDiff] No suitable packages found for diffing`);
    return null;
  }
